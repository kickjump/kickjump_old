...
export interface OutputWithCursor<TData, TCursor extends any = any> {
  cursor: TCursor | null;
  data: TData;
}
...
export function createReactQueryHooks<TRouter extends AnyRouter, TSSRContext = unknown>() {
  type TQueries = AssertType<AssertLegacyDef<TRouter>['queries'], ProcedureRecord>;
  type TSubscriptions = AssertType<AssertLegacyDef<TRouter>['subscriptions'], ProcedureRecord>;
  type TMutations = AssertType<AssertLegacyDef<TRouter>['mutations'], ProcedureRecord>;
    ...
    return (
      <Context.Provider
        value={{
          ...
          setQueryData: useCallback((...args) => queryClient.setQueryData(...args), [queryClient]),
          getQueryData: useCallback((...args) => queryClient.getQueryData(...args), [queryClient]),
  ...
  > {
    ...
    return __useMutation((input) => {
      ...
      return (client.mutation as any)(...getClientArgs([actualPath, input], opts));
    }, opts);
  }
  ...
  function useInfiniteQuery<TPath extends TInfiniteQueryNames & string>(
    ...
    const [path, input] = pathAndInput;
    const { client, ssrState, prefetchInfiniteQuery, queryClient } = useContext();

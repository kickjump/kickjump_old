<script lang="ts">
  ...
  $: displayedFields = columns.length
    ? columns.map((column) => (typeof column === 'string' ? column : column.field))
    : data[0]
    ? Object.keys(data[0])
    : [];
</script>

<table {...$$restProps} class:table-alternating={striped} class:table-hover={hoverable}>
  {#if showHeader}
    <thead>
      <tr>
          ...
          <td class={cellCLass}>
            {#if (tmpCol = hasContentFn(getColumnFromName(columnName)))}
              ...
              {:else}
                {tmpCol.content(val)}
              {/if}
            {:else if hasHTMLProp(getColumnFromName(columnName))}
              {@html val}
            {:else}
              {val}
            {/if}

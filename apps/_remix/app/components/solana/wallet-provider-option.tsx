...
export const WalletProviderOption: React.FC<Props> = ({ info, onInstall, onSelect }: Props) => {
  const mustInstall = typeof window !== 'undefined' && info.isInstalled?.() === false;
  const icon =
    typeof info.icon === 'string' ? (
      <Image src={info.icon} alt={`Icon for wallet ${info.name}`} />
    ) : (
      <info.icon />
    );
